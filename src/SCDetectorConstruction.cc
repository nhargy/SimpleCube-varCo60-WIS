...
void SCDetectorConstruction::ConstructHolder()
{
    ...
    // --- Source Tube --------------------------------------------------------
    G4double SourceTube_oD     = GetGeometryParameter("SourceTube_oD");
    G4double SourceTube_iD     = GetGeometryParameter("SourceTube_iD");
    G4double SourceTube_Height = GetGeometryParameter("SourceTube_Height");

    G4Tubs* solid_SourceTube = new G4Tubs(
        "solid_SourceTube",
        SourceTube_iD/2,
        SourceTube_oD/2,
        SourceTube_Height/2,
    ...
    logic_SourceTube = new G4LogicalVolume(
        solid_SourceTube,
        PLA,
        "logic_SourceTube"
    );

    G4double zpos_SourceTube = zpos_Plate - Plate_Thickness/2 + SourceTube_Height/2;
    auto vector_SourceTube = G4ThreeVector(0., 0., zpos_SourceTube);

    phys_SourceTube = new G4PVPlacement(
        0, 
        vector_SourceTube, 
        logic_SourceTube, 
        "phys_SourceTube", 
    ...
    logic_SourceTube ->SetVisAttributes(vis_PLA);
}

void SCDetectorConstruction::ConstructShielding(){

    // --- PbBox --------------------------------------------------------------
    G4double PbBox_Width     = GetGeometryParameter("PbBox_Width");
    G4double PbBox_Length    = GetGeometryParameter("PbBox_Length");
    G4double PbBox_Height    = GetGeometryParameter("PbBox_Height");
    G4double PbBox_Thickness = GetGeometryParameter("PbBox_Thickness");

    G4Box* solid_PbBox1 = new G4Box(
        "solid_PbBox1", 
        PbBox_Width/2, 
        PbBox_Length/2, 
        PbBox_Height/2
    ...
    G4Box* solid_PbBox2 = new G4Box(
        "solid_PbBox2", 
        (PbBox_Width/2)  - (PbBox_Thickness), 
        (PbBox_Length/2) - (PbBox_Thickness), 
        (PbBox_Height/2) - (PbBox_Thickness/2)
    );

    auto union_vector_PbBox = G4ThreeVector(0., 0., PbBox_Thickness/2);
    G4SubtractionSolid* solid_PbBox = new G4SubtractionSolid(
        "solid_PbBox", 
        solid_PbBox1, 
        solid_PbBox2,
        0,
    ...
    logic_PbBox = new G4LogicalVolume(
        solid_PbBox,
        Pb,
        "logic_PbBox"
    );
    ...
    G4double Plate_Thickness  = GetGeometryParameter("Plate_Thickness");
    G4double plate_bottom = zpos_Plate - Plate_Thickness/2;
    zpos_PbBox = plate_bottom + PbBox_Height/2  - PbBox_Thickness;
    auto vector_PbBox = G4ThreeVector(0., 0., zpos_PbBox);

    phys_PbBox = new G4PVPlacement(
        0, 
        vector_PbBox, 
        logic_PbBox, 
        "phys_PbBox", 
    ...
    // --- PbTube -------------------------------------------------------------
    G4double PbTube_oD     = GetGeometryParameter("PbTube_oD");
    G4double PbTube_iD     = GetGeometryParameter("PbTube_iD");
    G4double PbTube_Height = GetGeometryParameter("PbTube_Height");

    G4Tubs* solid_PbTube = new G4Tubs(
        "solid_PbTube",
        PbTube_iD/2,
        PbTube_oD/2,
        PbTube_Height/2,
    ...
    logic_PbTube = new G4LogicalVolume(
        solid_PbTube,
        Pb,
        "logic_PbTube"
    );

    G4double zpos_PbTube = plate_bottom + PbTube_Height/2;
    auto vector_PbTube = G4ThreeVector(0., 0., zpos_PbTube);

    phys_PbTube = new G4PVPlacement(
        0, 
        vector_PbTube, 
        logic_PbTube, 
        "phys_PbTube", 
    ...
    /* -- VisAttributes ---------------------------------------------------- */
    auto colour_Pb = G4Colour(1., 1., 1., 0.4);
    G4VisAttributes *vis_Pb = new G4VisAttributes(colour_Pb);
    vis_Pb   ->SetVisibility(true);
    //vis_Pb   ->SetForceSolid(true);
    vis_Pb   ->SetForceWireframe(true);
    vis_Pb   ->SetForceAuxEdgeVisible(true);
    logic_PbBox ->SetVisAttributes(vis_Pb);
    logic_PbTube ->SetVisAttributes(vis_Pb);
}
...
void SCDetectorConstruction::ConstructRoom()
{
    ...
    G4double PbBox_Height = GetGeometryParameter("PbBox_Height");
    G4double zpos_Floor = zpos_PbBox - PbBox_Height/2 - Floor_Thickness/2;
